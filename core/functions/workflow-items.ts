...
export default createHandler({
  ...
  async POST(req, ctx) {
    ...
    if (!body.workflow_definition_id || !body.title) {
      return error('workflow_definition_id and title required')
    }
  ...
  async PATCH(req, ctx, params) {
    ...
    if (isStageChange) {
      ...
      if (transitionDef) {
        ...
        if (allowedTransitions.length > 0 &&
            !allowedTransitions.includes(body.stage_definition_id)) {
    ...
    const eventType = isStageChange ? 'item.stage_changed' : 'item.updated'
    ...
    await emitActivity(ctx, eventType, activitySummary, 'item', id, transitionMeta)
    await emitOutboxEvent(ctx.accountId!, eventType, 'item', id, { before, after: data, ...transitionMeta })

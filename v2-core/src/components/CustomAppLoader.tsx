...
// Custom apps live in v2-custom/src/apps/{slug}/index.tsx (resolved via @custom alias).
// Core apps live in v2-core/src/apps/{slug}/index.tsx (relative import).
// We try custom first; if the module doesn't exist, fall back to core.
function getAppComponent(slug: string): React.LazyExoticComponent<React.ComponentType> {
  if (!appModuleCache.has(slug)) {
    const LazyComponent = lazy(() =>
      import(`@custom/apps/${slug}/index.tsx`).catch(() =>
        import(`../apps/${slug}/index.tsx`)
      )
    )

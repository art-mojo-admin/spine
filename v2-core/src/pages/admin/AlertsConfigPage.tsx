...
export function AlertsConfigPage() {
  ...
  const getMetricBadge = (metric: string) => {
    const colors: Record<string, string> = {
      ...
      latency_p95: 'bg-amber-100 text-amber-700',
      pipeline_failure_rate: 'bg-purple-100 text-purple-700',
    }
    return (
      <span className={`inline-flex px-2 py-0.5 text-xs font-medium rounded-md ${colors[metric] || 'bg-slate-100 text-slate-600'}`}>
    ...
    >
      {alerts && alerts.length === 0 ? (
        <div className="p-8 text-center">
          ...
          <h3 className="mt-2 text-sm font-medium text-slate-900">No alerts configured</h3>
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-4 mt-2 text-sm text-slate-600">
                    ...
                    {alert.data?.pipeline_id && (
                      <>
                        <span>·</span>
                        <span className="text-navy">Pipeline: {alert.data.pipeline_id.slice(0, 8)}...</span>
                      </>
                    )}
                  </div>
                </div>
                <div className="flex items-center gap-2 ml-4">
                  ...
                  <button
                    ...
                    className="p-2 text-slate-400 hover:text-navy hover:bg-navy/10 rounded-lg transition-colors"
                    title="Edit"
                  >
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <input
            ...
            required
            className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-navy focus:border-navy"

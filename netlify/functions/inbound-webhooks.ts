...
async function executeMapping(
  ...
  switch (mapping.action) {
    case 'transition_item': {
      ...
      await emitActivity(ctx, 'workflow_item.stage_changed', `"${updated.title}" transitioned via inbound webhook`, 'workflow_item', itemId, { ...transitionMeta, source: 'inbound_webhook' })
      await emitOutboxEvent(accountId, 'workflow_item.stage_changed', 'workflow_item', itemId, { before: item, after: updated, ...transitionMeta, source: 'inbound_webhook' })
    ...
    case 'update_item_field': {
      ...
      await emitActivity(ctx, 'workflow_item.updated', `Updated item ${itemId} via inbound webhook`, 'workflow_item', itemId, { fields: Object.keys(updates), source: 'inbound_webhook' })
    ...
    case 'create_item': {
      ...
      const fields: Record<string, any> = {
        ...
        workflow_type: config.workflow_type || 'default',
      ...
      const { data, error: dbErr } = await db.from('workflow_items').insert(fields).select().single()
      if (dbErr) return { success: false, detail: dbErr.message }
      ...
      await emitActivity(ctx, 'workflow_item.created', `Created "${data.title}" via inbound webhook`, 'workflow_item', data.id, { source: 'inbound_webhook' })
      await emitOutboxEvent(accountId, 'workflow_item.created', 'workflow_item', data.id, { ...data, source: 'inbound_webhook' })

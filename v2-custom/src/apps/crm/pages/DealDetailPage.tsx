...
export default function DealDetailPage() {
  ...
  useEffect(() => {
    if (isNew) return
    apiFetch(`/api/items?action=get&id=${id}`)
      .then(r => r.json())
      .then(json => {
        if (json) {
          setForm({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      let res: Response
      if (isNew) {
        res = await apiFetch('/api/items?action=create', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
      } else {
        res = await apiFetch(`/api/items?action=update&id=${id}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
      }
  ...
  const handleDelete = async () => {
    ...
    await apiFetch(`/api/items?action=delete&id=${id}`, { method: 'POST' })
    navigate('/crm/deals')
  }
  ...
  return (
    <div className="p-6 max-w-2xl">
      <div className="flex items-center gap-3 mb-6">
        <button onClick={() => navigate('/crm/deals')} className="text-slate-400 hover:text-slate-700 text-sm">
          ← Deals
        </button>
      ...
      <div className="flex items-center justify-between mt-6">
        ...
        <div className="flex gap-3">
          <button onClick={() => navigate('/crm/deals')} className="px-4 py-2 text-sm text-slate-600 border border-slate-200 rounded-lg hover:bg-slate-50">
            Cancel
          </button>
          <button
            onClick={handleSave}

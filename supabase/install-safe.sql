-- ============================================================================
-- Spine: SAFE / Idempotent Installation SQL
-- Safe to run against an existing Supabase project — every statement uses
-- IF NOT EXISTS, CREATE OR REPLACE, or DROP-then-CREATE guards.
--
-- Usage:
--   Run this in the Supabase SQL Editor on a new OR existing project.
--   Re-running is harmless; it will skip objects that already exist.
--
-- What's included:
--   - Extensions (uuid-ossp, pgcrypto, vector, pg_trgm)
--   - All tables, indexes, triggers, and functions
--   - RLS policies and security lockdown
--   - Template packs (CRM, Support, Recruiting, etc.)
--
-- What's NOT included:
--   - Seed data (KB articles, demo accounts)
--   - Auth users (created via Supabase Auth)
-- ============================================================================
...
-- Move extensions to extensions schema (safe if already moved)
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'vector'
             AND extnamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public'))
  THEN ALTER EXTENSION vector SET SCHEMA extensions;
  END IF;
END $$;
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_trgm'
             AND extnamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public'))
  THEN ALTER EXTENSION pg_trgm SET SCHEMA extensions;
  END IF;
END $$;

-- ════════════════════════════════════════════════════════════════════════════
-- 031: RLS POLICIES (drop-if-exists + create)
-- ════════════════════════════════════════════════════════════════════════════
...
-- ── Drop all Spine RLS policies so they can be (re)created cleanly ──────
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'config_packs_name_key'
  ) THEN
